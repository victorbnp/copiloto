...
tolerancia = 0.05  # 10% de tolerância para amplitudes/mínimas/máximas
...
# Simular resultados das operações (com novas regras)
for op in operacoes:
    ...
    for _, row in sub_df.iterrows():
        if op['direcao'] == "COMPRA":
            ...
            if row['low'] <= op['stop_loss']:
                ...
                break
            # Verifica se atingiu 1º Take Profit (trava stop na entrada)
            elif not parcial_atingida and row['high'] >= op['take_profit1']:
                parcial_atingida = True
                op['stop_loss'] = op['entrada']  # Move SL para break-even
            # Verifica se atingiu 2º Take Profit
            elif parcial_atingida and row['high'] >= op['take_profit2']:
                op['resultado'] = 2  # Ganho 2x risco (1x parcial + 1x final)
                break
        else:  # VENDA
            if row['high'] >= op['stop_loss']:
                ...
                parcial_atingida = True
                op['stop_loss'] = op['entrada']  # Move SL para break-even
            elif parcial_atingida and row['low'] <= op['take_profit2']:
                op['resultado'] = 2  # Ganho 2x risco
                break

    # Se apenas o 1º TP foi atingido (sem fechar no 2º TP)
    if parcial_atingida and op['resultado'] is None:
        op['resultado'] = 1  # Ganho 1x risco (parcial)

# Cálculo do Profit Factor Ponderado
ganho_total = 0
perda_total = 0

for op in operacoes:
    if op['resultado'] > 0:
        ganho_total += op['resultado']  # 1 ou 2, proporcional ao risco
    else:
        perda_total += abs(op['resultado'])  # 1 (risco fixo)

profit_factor = ganho_total / perda_total if perda_total != 0 else float('inf')

# Estatísticas
print(f"Total de operações: {len(operacoes)}")
print(f"Win Rate: {(len([op for op in operacoes if op['resultado'] > 0]) / len(operacoes)) * 100:.2f}%")
print(f"Profit Factor Ponderado: {profit_factor:.2f}")
